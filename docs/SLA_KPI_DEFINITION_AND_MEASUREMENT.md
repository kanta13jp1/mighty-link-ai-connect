...
### 2.1.1 利用状況アナリティクス KPI（T800）

2026-06-27にT800として、匿名イベント計測を `POST /api/analytics/event` と `usage_analytics_events` に導入した。Firebase Analytics SDKの自動収集はまだ有効化せず、公開デモでは個人情報を保存しないファーストパーティ計測を正本とする。

| KPI | 定義 | 計測頻度 | 目標（パイロット期） |
| :--- | :--- | :--- | :--- |
| 利用イベント数 | `usage_analytics_events` の総件数 | 日次 | デモ操作ごとに増加 |
| 7日間利用イベント数 | 過去7日間のイベント件数 | 週次 | 主要機能確認時に1件以上 |
| 7日間匿名セッション数 | 過去7日間の `session_pseudonym` distinct件数 | 週次 | 社内確認参加者数と概ね一致 |
| ページ表示数 | `event_name = 'page_view'` の件数 | 日次 | デモ確認ごとに増加 |

プライバシー制御:

- IPアドレス、生User-Agent、フォーム本文、氏名、メールアドレス、電話番号、URLクエリ文字列を保存しない。
- ブラウザlocalStorageの匿名セッションIDはサーバー側で `usage-<digest>` に疑似ID化する。
- 管理ダッシュボード `GET /api/admin/operations-dashboard` とCSV exportにT800 KPIを表示する。
