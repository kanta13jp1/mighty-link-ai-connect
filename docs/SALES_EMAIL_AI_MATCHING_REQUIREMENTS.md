...
- 作成日: 2026-06-17
- 関連WBS: T817, T817_1, T817_2, T817_3, T817_4, T817_5, T817_6, T817_7, T821, T824, T920, T923
...
- T920で公開マッチング進捗へ、フリーワード、案件の必須スキル、単価下限/上限、適合度のリアルタイム絞り込みと件数表示、条件リセットを実装する。
- 単価条件は案件単価レンジとの重なりで判定し、単価未抽出案件は単価指定時に除外する。フリーワードは構造化項目と許可済み送信元ドメインを対象とし、メール本文全文は検索・表示しない。
- T923で案件メールの受信日を一覧へ表示し、受信日の開始日・終了日を指定できる期間フィルターを追加する。RFC 2822またはISO形式の受信日時はJSTの `YYYY-MM-DD` へ正規化し、期間指定時は日付不明の案件を除外する。
- 受信日フィルターはAPIで候補の上限件数を適用する前に評価し、開始日が終了日より後の場合や実在しない日付は `400` で拒否する。
