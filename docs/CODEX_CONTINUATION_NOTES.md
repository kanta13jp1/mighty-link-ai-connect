...
## 2026-05-22 Codex continuation: T672 Seedance saved default and cost guard

- Downloaded the successful Seedance output from task `cgt-20260522194932-9g599` and saved it as `exports/seedance_demo/mighty_skill_bridge_seedance_demo.mp4`.
- Preserved the previous procedural fallback video as `exports/seedance_demo/mighty_skill_bridge_procedural_fallback.mp4`.
- Added a `Download Video` control to the demo UI.
- Added `SEEDANCE_API_ENABLED`; external Seedance billing calls are disabled by default even when API credentials are present.
- Documented BytePlus ModelArk usage/resource-pack checks as the spend source of truth.

## 2026-05-22 Codex continuation: T673 External API guard dashboard

- Added local admin dashboard `/admin` and JSON endpoint `/api/admin/usage`.
- Added gitignored local ledger `data/external_api_usage.jsonl` for external API events without prompts, keys, or signed URLs.
- Added circuit breakers for Seedance generation (`SEEDANCE_DAILY_GENERATION_LIMIT`, default 1) and Gemini live calls (`GEMINI_DAILY_CALL_LIMIT`, default 20).
- Added provider-reported token extraction when responses include usage metadata; documented that BytePlus ModelArk Usage remains authoritative for Seedance spend.
- Reflected the work in WBS, issue tracker, QA tracker, setup guide, and project structure docs.

## 2026-05-22 Codex continuation: T674 Favicon and route polish

- Generated a branded root `favicon.ico` for the public demo and local FastAPI screens.
- Added FastAPI `/favicon.ico` serving and linked the favicon from `index.html`, `src/index.html`, and the `/admin` dashboard.
- Added `/admin/usage` as a browser-friendly alias for `/api/admin/usage`.
- Migrated the Gemini import path from deprecated `google-generativeai` to current `google-genai`.
- Switched local Windows startup to `WindowsSelectorEventLoopPolicy` before Uvicorn starts to reduce noisy MP4 partial-request disconnect traces.

## 2026-05-23 Codex continuation: T675 Chrome DevTools workspace route

- Added the local FastAPI route `/.well-known/appspecific/com.chrome.devtools.json`.
- The route returns Chrome DevTools workspace JSON with the absolute local project root and a stable deterministic UUID.
- Documented that this is a localhost development convenience route and should not be represented as a public GitHub Pages static asset.

## 2026-05-23 Codex continuation: T676 Seedance風ナビ/フッター刷新

- Updated `scripts/render_seedance_video_demo_ui.py` as the single source for `index.html` and `src/index.html`.
- Added Seedance-style top navigation: `Home`, `Models`, `Blog & Publication`, `Join Us`, plus `EN / JP` and API status.
- Added scroll behavior that toggles `.topbar.scrolled`, making the header more compact after the first viewport starts moving.
- Added a `Models` band and `Models` / `Teams` / `Learn More` footer while keeping the Mighty Skill-Bridge brand, saved Seedance video, `Generate Video`, `Download Video`, API Guard link, and Knowledge Flow links.
- Playwright verification saved:
  - `exports/verification/seedance_nav_desktop.png`
  - `exports/verification/seedance_nav_mobile.png`
- Verification confirmed no horizontal overflow on 1440px desktop or 390px mobile, video source present, download link present, footer present, and scroll-state class applied.

## 2026-05-23 Codex continuation: T677 Sheetsガント風タイムライン化

- Updated `scripts/sync_wbs_to_sheets.py` so the `WBS Timeline` tab is generated as a Gantt-style date grid instead of a simple task table.
- The timeline now includes left frozen columns, month headers, daily columns, weekend shading, a blue today marker, status-colored bars, row filters, and dynamic column sizing.
- Kept `data/WBS.tsv` as the single source of truth; the visual schedule is rebuilt from WBS start/end dates during Sheets sync.
- Reflected the work in `data/WBS.tsv`, `docs/WBS.md`, `data/issues_tracker.tsv`, and `data/qa_tracker.tsv`.

## 2026-05-23 Codex continuation: T678 Sheets遅延タスク可視化

- Added a `遅延` column to the generated `WBS Timeline` tab.
- Added schedule risk detection in `scripts/sync_wbs_to_sheets.py`: `終了遅れ`, `着手遅れ`, `期限間近`, and `正常`.
- Late rows are highlighted with a light orange background, due-soon rows with light yellow, and delayed Gantt bars use a stronger orange/yellow accent.
- Kept the implementation aligned with Google Sheets API guidance by using batched formatting updates and conditional formatting rules rather than per-cell API calls.

## 2026-05-23 Claude Code Canva/Figma リデザイン docs 完遂

ユーザー (2026-05-23) より「CEOプレゼンPPTX (Google Slide互換表示)ですが、FigmaかCanvaのTemplateで作成し直すことはできますか?」と要望。Claude Code レーン (docs / コピペカード生成) で対応、実 Canva 操作は人間 + Antigravity の handoff (HANDOFF-14) に切り出した。

### 6th session — Canva/Figma docs 完遂内容

- 新規 [docs/CEO_PRESENTATION_CANVA_FIGMA_GUIDE_2026-06-02.md](CEO_PRESENTATION_CANVA_FIGMA_GUIDE_2026-06-02.md) (T658-extend deliverable):
  - Canva PPTX import 10 ステップ手順 (所要 30 分)
  - Figma Slides 代替パス
  - **Mighty Skill-Bridge ブランドカラー 7 色定義** (cyber black `#0d0e15` / neon blue `#00f0ff` / neon green `#39ff14` 等、Seedance シネマティック UI と統一)
  - **全 8 枚コピペ専用カード** (タイトル / 要点 3 つ / 話すメモ / 見せる証跡 / 社長への質問 / デザインメモ) — 元データは [exports/knowledge_flow/notebooklm_ceo_slide_outline.md](../exports/knowledge_flow/notebooklm_ceo_slide_outline.md) (2026-05-23 22:23 生成)
  - 公開デモスクショ差し込み手順 (スライド 2-4 用、3 枚)
  - 実行チェックリスト + Canva インポート不具合時の Plan B (B-1〜B-4)
- [data/issues_tracker.tsv](../data/issues_tracker.tsv) に HANDOFF-14 / HANDOFF-15 / HANDOFF-16 を追加
- [docs/CEO_PRESENTATION_PREP_2026-06-02.md](CEO_PRESENTATION_PREP_2026-06-02.md) 関連 docs リストに CANVA_FIGMA_GUIDE 追加
- [README.md](../README.md) Documents セクションに CANVA_FIGMA_GUIDE 追加

### 6th session — Canva docs / WBS.tsv 反映依頼 (Codex セッションへ handoff)

- **T658-extend** (Canva/Figma リデザイン手順 docs) を新規 WBS 行として data/WBS.tsv に追加: ステータス `完了` / 担当 `Claude Code` / 実行エンジン `VSCode + Claude Code` / 開始 `2026-05-23` / 終了 `2026-05-23`
- 既存依頼 (T605/T606/T607/T614/T615 完了 flip / T663 実行中化) と一括 commit 推奨

### 6th session — Canva docs / Codex への handoff キュー追加

- **HANDOFF-14**: 人間 + Antigravity が Canva PPTX import + テンプレ適用 + 公開デモスクショ埋込み (CANVA_FIGMA_GUIDE に従う、所要 30 分、期限 5/30)
- **HANDOFF-15** (任意): `scripts/generate_ceo_presentation_deck.py` に `--style canva-export` オプション追加
- **HANDOFF-16** (任意): `scripts/capture_demo_screenshots.py` 新規作成 (Playwright で 1920x1080 スクショ自動取得)

### 6th session — Light refresh (2026-05-23)

- Anthropic / OpenAI / Google: 公式 docs 変更なし (前回 fetch から < 24h、本日中の知見で十分)
- 次回は 2026-05-24 (5/27 quota refresh 後と合わせて再 fetch)

### 6th session — Stale-doc audit (2026-05-23)

- 該当なし。前セッションで R1 訂正 (Gemini 3.5 Pro 来月予約 → I/O 2026 確定情報) 完了済、本日 Codex が R23-R27 を全て resolved に更新済

## 2026-05-23 (7th session) Claude Code MCP 自動化 docs 完遂

ユーザー (2026-05-23) より「Figma MCPや、Canva MCPを使って手動作業を自動化できませんか?」と要望。verify-first で両 MCP の公式提供を確認 → 自動化フロー docs を新規作成し、HANDOFF-14 を MCP 版に upgrade。

### 7th session — MCP 自動化 完遂内容

- 新規 [docs/MCP_CANVA_FIGMA_SETUP_GUIDE_2026-06-02.md](MCP_CANVA_FIGMA_SETUP_GUIDE_2026-06-02.md) (T658-mcp-extend deliverable):
  - Canva MCP (`https://mcp.canva.com/mcp`) Claude Code セットアップ手順 (npx mcp-remote + OAuth DCR)
  - Figma MCP (`https://mcp.figma.com/mcp` Remote / `http://127.0.0.1:3845/mcp` Dev Mode local) セットアップ
  - **プラン別機能マトリクス**: 全プラン (生成/編集/エクスポート OK) / Pro+ (リサイズ) / Enterprise (ブランドキット autofill)
  - Claude Code から 1 ターンで 8 枚スライド自動生成するプロンプト (Canva 版 + Figma 版)
  - 7-step 自動化フロー (Step A 認証確認 → Step G FINAL_REVIEW_CHECKLIST 更新)
  - フォールバック表 (OAuth 詰まり / ネットワーク / 機能不足 / 時間切れ)
- [docs/CEO_PRESENTATION_CANVA_FIGMA_GUIDE_2026-06-02.md](CEO_PRESENTATION_CANVA_FIGMA_GUIDE_2026-06-02.md) に「§0 MCP 自動化版あり」セクション追加 (手動版は MCP フォールバック扱い)
- [data/issues_tracker.tsv](../data/issues_tracker.tsv):
  - HANDOFF-14 → **HANDOFF-14a (手動、LOW)** + **HANDOFF-14b (MCP 自動化、MED)** に分岐
  - 新規 **HANDOFF-17**: Canva MCP セットアップ (寛太、claude mcp add canva、10 分、期限 5/25)
  - 新規 **HANDOFF-18**: Figma MCP セットアップ (寛太、Canva 失敗時のバックアップ、10 分、期限 5/25)
- [docs/CEO_PRESENTATION_PREP_2026-06-02.md](CEO_PRESENTATION_PREP_2026-06-02.md) 関連 docs リストに MCP_CANVA_FIGMA_SETUP_GUIDE 追加
- [README.md](../README.md) Documents セクションに MCP_CANVA_FIGMA_SETUP_GUIDE 追加

### 7th session — WBS.tsv 反映依頼 (Codex セッションへ handoff)

- **T658-mcp-extend** (Canva/Figma MCP セットアップ + 自動化 docs) を新規 WBS 行として `data/WBS.tsv` に追加: ステータス `完了` / 担当 `Claude Code` / 開始 `2026-05-23` / 終了 `2026-05-23`
- 既存依頼 (T605/T606/T607/T614/T615/T658-extend 完了 flip / T663 実行中化) と一括 commit 推奨

### 7th session — Codex への handoff キュー追加

- **HANDOFF-17 / HANDOFF-18**: 人間 (寛太) が Canva / Figma MCP セットアップ実行 (期限 5/25)。Codex は support 役
- **HANDOFF-14b**: HANDOFF-17 完了後、Claude Code セッションで Canva MCP プロンプトを実行し PPTX/PDF 自動生成 (期限 5/26)

### 7th session — verify-first 公式 docs 確認結果 (AI-TOOL-VERIFY)

- **Canva MCP**: 公式提供 [canva.dev/docs/mcp](https://www.canva.dev/docs/mcp/) 確認済、エンドポイント `https://mcp.canva.com/mcp`、全プラン (無料含む) で design 生成/編集/エクスポート可能、ブランドキット autofill は Enterprise 限定
- **Figma MCP**: 公式提供 [developers.figma.com/docs/figma-mcp-server](https://developers.figma.com/docs/figma-mcp-server/) 確認済、Remote 推奨、Beta 期間中無料 (将来 usage-based 課金)、Figma Slides 専用コマンドは未提供 (通常 Frame で代替)

[AI-TOOL-VERIFY] 規約 (2026-05-07 #1706) に従い、両 MCP claim を公式 docs で確認済

## 2026-05-23 (8th session) Claude Code MCP setup guide bugfix

寛太が HANDOFF-17 を実機実行した際、7th session で書いた MCP セットアップコマンドが誤りでエラーになった。即修正。

### 8th session — 実機検証で発見したエラー
...
[code.claude.com/docs/en/mcp](https://code.claude.com/docs/en/mcp) の公式 `claude mcp add` 構文は:

- HTTP: `claude mcp add --transport http <name> <url>`
- stdio: `claude mcp add [options] <name> -- <command> [args...]` (`--` セパレーター必須)

7th session docs では非公式な `--transport stdio --command --args` (実在しないフラグ) を使っていた。

### 8th session — 修正内容

- [docs/MCP_CANVA_FIGMA_SETUP_GUIDE_2026-06-02.md](MCP_CANVA_FIGMA_SETUP_GUIDE_2026-06-02.md):
  - §1.2 Canva セットアップを公式構文に書き直し (HTTP transport 推奨 + stdio フォールバック)
  - §1.2.1 認証フロー追加 (`claude mcp add` 後に `/mcp` で OAuth 認可)
  - §2.2 Figma セットアップも同様に修正
  - **新規 §4.5 よくあるエラーと修正** — 寛太が遭遇した 4 つの実エラー + 修正手順
- 更新履歴に bugfix エントリ追加

### 8th session — verify

公式 [Claude Code MCP docs](https://code.claude.com/docs/en/mcp) を WebFetch で確認、構文例を直接引用:

> `claude mcp add --transport http stripe https://mcp.stripe.com`
> `claude mcp add --transport stdio --env AIRTABLE_API_KEY=YOUR_KEY airtable -- npx -y airtable-mcp-server`

「All options must come **before** the server name. The `--` (double dash) then separates the server name from the command and arguments」と明記されている。

### 8th session — 寛太への正しいコマンド

```bash
# Canva MCP 追加 (HTTP 推奨)
...
# セッション内で認証
> /mcp
# canva の Authenticate を選択 → ブラウザで OAuth 認可
