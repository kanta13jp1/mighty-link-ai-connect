...
社内教育担当により策定された役割別の研修ハンドブックは以下を参照してください。

| コース | 対象 | ドキュメント | 主な講義内容 |
| --- | --- | --- | --- |
| **コース① 全社員基礎** | 全社員・一般利用者 | [全社員基礎研修ハンドブック](training/FOUNDATION_TRAINING_HANDBOOK.md) | 初期セットアップ、アカウント有効化、適性アンケート、勤怠打刻・CSV解析 |
| **コース② 営業・HR専門** | 営業担当・HR/採用担当 | [営業・HR専門研修ハンドブック](training/SALES_HR_TRAINING_HANDBOOK.md) | 4軸AIフィット分析、営業メールAIマッチング、個人情報保護・AI補足利用原則 |
| **コース③ 管理者管理** | システム管理者・事業責任者 | [管理者管理研修ハンドブック](training/ADMIN_MANAGEMENT_TRAINING_HANDBOOK.md) | 統合ダッシュボード操作、仮名化ログ点検、アカウント管理、セキュリティ・障害対応 |
...
| --- | --- | --- |
| ホーム | `#top` | サービス概要と主要導線 |
| 初期セットアップ | `#onboarding-section` | オンボーディング手順とアカウント有効化 |
...
### 2.1.1 初期セットアップ / オンボーディングとアクティベーション (T752)

社内利用は**管理者がアカウントを発行**する運用です。利用者は自己サインアップではなく、
発行済みアカウントで `#onboarding-section`（初期セットアップ）の手順を完了し、
**アクティベーション（有効化）**を行ってから利用を開始します。

セットアップ手順は画面が独自に持つのではなく `GET /api/onboarding/state` から取得します。
UI と有効化APIの判定条件が同じ定義を参照するため、両者がずれることはありません。

| ステップ | 区分 | 内容 |
| --- | --- | --- |
| アカウントの受領とサインイン | 必須 | 管理者が発行したアカウントでサインインし、本人のメールアドレスであることを確認します |
| 利用規約・プライバシーポリシーへの同意 | 必須 | 画面上部の同意欄で、利用規約・プライバシーポリシー・特商法表記・課金規約に同意します |
| 表示名と所属の初期設定 | 必須 | 社内で識別できる表示名と所属部署を設定します（氏名などの個人情報は入力不要） |
| スキルシートで初回診断を試す | 任意 | サンプル等でフィット分析を一度実行し、結果の見方を確認します |
| 利用ガイドとサポート窓口の確認 | 任意 | 本ガイドとサポート窓口の連絡先を確認します |

**有効化の条件**（`POST /api/onboarding/activate`）:

1. 必須ステップがすべて完了していること（未完了があると 400 で拒否し、残りのステップ名を返します）
2. 利用規約等に同意済みで、同意バージョンが現行と一致すること（旧版・未同意は 400）
3. アカウント識別子が3文字以上であること

**プライバシー**: 進捗はブラウザの `localStorage`（`msb_onboarding_progress_v1`）にのみ保持し、
サーバへは利用者ごとの行を保存しません。有効化時の監査ログには**仮名化ID**のみを記録し、
入力したアカウント識別子そのものは保存しません。

> 公開デモ（GitHub Pages）は静的配信でAPIが無いため、ウィザードは同内容の内蔵定義で動作し、
> 有効化は画面内の確認のみになります。実際の有効化と監査記録は社内環境で行われます。
