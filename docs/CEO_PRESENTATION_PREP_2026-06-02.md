...
- **UI Wireframes — 10 Patterns (2026-05-24 生成)**: [Mighty Skill-Bridge UI Wireframes 2026-06-02](https://docs.google.com/presentation/d/1qTdOWsLhUf0GzVDkztiuuLczcYkQuQfo/edit?usp=drivesdk) — 社長判断材料用の 10 種類 UI レイアウトパターン (Vertical Hero / Split Form / Step Wizard / Chat / DnD / Kanban / Comparison / Dashboard / Inline Live / Search Catalog)。greyscale wireframe で「画面設計のパターン選択」に集中。Figma companion: <https://www.figma.com/design/aiQt3c1Cenru4x6GMcLuL5> (Starter plan rate limit 解除後に同内容を MCP で構築予定)。
...
## 2026-05-22 Seedance動画デモUI刷新

- 公開URLの第一印象を改善するため、`index.html` と `src/index.html` のファーストビューを動画生成デモUIへ刷新した。
- Seedanceの質感は、強いタイポグラフィ、映像AIプロダクト風のプレビューフレーム、グリッド表現のみを参考にし、ブランド・素材・文言はMighty Skill-Bridge独自のまま維持した。
...
## 2026-05-22 Seedance API動画デモ接続

- `src/app.py` に `/api/seedance/video-demo` を追加し、環境変数設定時はSeedance APIへ接続できるようにした。
- 公開URLは静的配信のため、生成済みMP4 `exports/seedance_demo/mighty_skill_bridge_seedance_demo.mp4` を表示する。
- 実API確認に必要な `SEEDANCE_API_KEY` / `SEEDANCE_API_URL` / `SEEDANCE_RESULT_API_URL_TEMPLATE` は `docs/SETUP_GUIDE.md` に記録した。
- WBS `T668`、課題 `R17`、QA `Q-AHOC-20260522-4` に反映した。
