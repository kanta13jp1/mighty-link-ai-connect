...
## 2026-06-12 update: Calendar dynamic WBS events

- `scripts/sync_wbs_to_calendar.py` now generates one all-day Calendar event per non-completed `data/WBS.tsv` row that is not covered by the static `SCHEDULE_EVENTS` list (title: `【Mighty Skill-Bridge】<タスクID> <タスク名>`).
- Rescheduling therefore only requires updating the TSV and re-running the sync; events for rows flipped to `完了` are deleted automatically via the `wbsIds` private extended property.
