...
| **T817** | 7. 決定後実行 | バックエンド開発 | 営業メールの自動解析・DB化およびAIマッチング機能の本格実装 | 寛太梅澤 + Codex | VSCode + Codex + Gemini API + Supabase + Gmail API | 2026-06-17打ち合わせで最優先化。docs/SALES_EMAIL_AI_MATCHING_REQUIREMENTS.mdを正本に、共有営業アドレスのメール取り込み、AI抽出、Supabase案件/要員DB、双方向検索、候補者マッチング、人間レビュー、本番hardeningをT817_1〜T817_7で実施 | 実行中 |
...
| **T817_2** | 7. 決定後実行 | バックエンド開発 | Gmail/ファイル取り込みPoCと重複排除 | Codex | VSCode + Codex + Gmail API | 共有営業アドレスのGmail APIまたは.eml/.txt/CSV取り込みでテストメールを取得し、件名/送信者/本文ハッシュによる重複排除とsecret非保存を検証する | 未着手 |
