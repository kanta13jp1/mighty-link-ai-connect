...
| **T847** | 8. 本番運用・品質管理 | コンプライアンス | 本番データ保持・削除・匿名化ポリシーの全テーブル実装照合 | Codex + Claude Code | VSCode + Codex + Claude Code + Supabase | employee_assessment_responses、attendance_*、sales_email_*、billing/support/user export/deletion系を含む全テーブルについて、保持期間、削除/匿名化、RLS、監査ログ、CSV原本/メール本文/secret非保存を確認し、USER_DATA_DELETION_FLOWとLOG_ROTATION_AND_RETENTION_RUNBOOKへ差分を反映する。T846前倒し完了を受け、次の横断文書ゲートとして前倒し。Issue #134 / Project #1 Todo | 未着手 |
| **T848** | 8. 本番運用・品質管理 | 外部連携 | AIモデル・外部SaaS・連携サービス棚卸しとGA時点利用可否の最終凍結 | Codex + Claude Code | VSCode + Codex + Claude Code + official docs | Anthropic/OpenAI/Google/Microsoft/Meta/Amazon/Apple/xAI/Kimi/MiMo/DeepSeek/BytePlus/GitHub/Slack/Notion/Obsidian/Unity/Figma/Canva/Reddit/InsForge/FireCrawl/Discord/Stripe/Supabase/お名前.comの利用有無、契約、モデル名、API version、fallback、廃止予定、secret管理、会社請求移管状態を棚卸しし、GA時点の採用/非採用を凍結する。T846前倒し完了を受け、外部連携棚卸しを前倒し。Issue #135 / Project #1 Todo | 未着手 |
| **T849** | 9. 長期保守・拡張 | 共通管理 | サイト開発完了総合判定・WBS全完了証跡化・GAリリース閉鎖 | Codex + Claude Code + 人間 | VSCode + Codex + Claude Code + GitHub + Google Workspace | 全WBSが完了、全公開前Go/No-GoがPASSまたは承認済み、GitHub Issues/Project未完了0、data/issues_tracker.tsvの開発必須open 0、data/qa_tracker.tsvの未回答/未承認QA 0、Sheets/Calendar同期済み、main/master/GA tag/GitHub Release/Firebase本番/NotebookLM/Driveが一致していることを確認し、このタスク完了をもってサイト開発完了と宣言する。T846前倒しに合わせ、最終判定日を前倒し。Issue #136 / Project #1 Todo | 未着手 |
| **T850** | 8. 本番運用・品質管理 | 運用引継ぎ | 会社運用引継ぎリハーサル・権限棚卸し・Break-glass確認 | 寛太梅澤 + Codex + Claude Code | 人間 + VSCode + Codex + Google Workspace | T823の会社アカウント移管後、GitHub/Firebase/GCP/Supabase/Stripe/Google Workspace/AI開発ツールの管理者、請求、退職時権限削除、緊急時Break-glass、運用Runbookに沿った復旧/問い合わせ対応を会社側でリハーサルし、属人化を解消する。T823終了予定に合わせて前倒し。Issue #137 / Project #1 Todo | 未着手 |
