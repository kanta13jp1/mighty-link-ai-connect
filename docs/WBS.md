...
| **T838** | 7. 決定後実行 | 調査/法務 | 従業員適性・状態把握ツールのショートリストPoC計画と法務確認 | Codex + Claude Code + 寛太梅澤 | VSCode + Codex + Claude Code + official vendor docs | T832の結果から2から3サービスをショートリスト化し、契約、個人情報、API/CSV連携、見積、権限管理、削除SLA、PoC評価基準を確認する。R36の法務レビューと接続し、R32のAI適性状況診断ツール実装範囲を再定義する | 未着手 |
