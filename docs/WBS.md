...
| **T800** | 8. 本番運用・品質管理 | 運用保守 | 利用状況アナリティクス計測設計と導入（イベント計測・KPI集計） | Codex | VSCode + Codex | Firebase Analytics / Supabase イベント計測の設計・導入結果を KPI ダッシュボードと Sheets へ接続。T805前倒し完了により2026-07-02開始へ前倒し | 未着手 |
...
| **T811** | 9. 長期保守・拡張 | DB保守 | Supabase Postgres 14 サポート終了（2026-07-01）対応：本番/staging の PG バージョン確認とアップグレード計画 | Codex | VSCode + Codex | Supabase 公式 changelog（2026-05-12）準拠。T828で整備したsecret非出力ゲートを使い、会社管理経路で受け取ったDB URLまたはSQL Editorのversion()結果でstaging/productionのPGメジャーバージョンを確認し、PG14ならアップグレード計画を実施 | 未着手 |
...
| **T817** | 7. 決定後実行 | バックエンド開発 | 営業メールの自動解析・DB化およびAIマッチング機能の本格実装 | 寛太梅澤 + Codex | VSCode + Codex + Gemini API + Supabase + provider-neutral mail connector | 2026-06-17打ち合わせで最優先化。docs/SALES_EMAIL_AI_MATCHING_REQUIREMENTS.mdを正本に、共有営業アドレスのメール取り込み、AI抽出、Supabase案件/要員DB、双方向検索、候補者マッチング、人間レビュー、本番hardeningをT817_1〜T817_7で実施。T824でGmail決め打ちを補正し、受信環境ヒアリング後にMicrosoft Graph/Gmail API/IMAP/Webhook等から接続方式を選定する | 実行中 |
...
| **T831** | 7. 決定後実行 | 運用管理 | 小林雅水社長のGoogleドライブへ保存された録画ファイルの整理・移動（R30対応） | 寛太梅澤 | 人間 | 適切な共有フォルダへ移動・散逸リスク解消 | 未着手 |
...
| **T834** | 7. 決定後実行 | セキュリティ | ホームページ更新完了に伴うWordPress・FTP一時認証情報の安全なクリーンアップと削除（R74対応） | 寛太梅澤 | 人間 | 認証情報クリーンアップ・パスワード無効化（社長連携） | 未着手 |
...
| **T836** | 7. 決定後実行 | 連携 | 顧客受信メール環境（Microsoft 365/Workspace等）の実接続設定とOAuth管理者承認取得 | 寛太梅澤 + Codex | Google/Microsoft Admin Console + Supabase | ヒアリング結果に基づくAPI接続（Graph/Gmail API等）の確立とテナント管理者承認完了 | 未着手 |
| **T837** | 9. 長期保守・拡張 | DB保守 | Supabase Postgres データベースのメジャーバージョンアップグレード実行と移行検証（R53対応） | Codex + 人間 | Supabase Dashboard + CLI | StagingおよびProduction環境のPostgres 15以上への無停止（または短時間メンテナンス）アップグレードとRLS/データ整合性検証 | 未着手 |
