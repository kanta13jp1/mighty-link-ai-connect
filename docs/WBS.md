...
| **T810** | 8. 本番運用・品質管理 | インシデント運用 | 障害インシデント対応記録・ポストモーテムテンプレートの整備 | Claude Code | VSCode + Claude Code | R44（本番502障害）を題材にポストモーテム雛形と記録ルールを docs 化し、DR Runbook（T749）と接続。再発防止アクションは課題管理表へ連携 | 未着手 |
...
| **T812** | 7. 決定後実行 | リリース運用 | 本番リリースのロールバック手順書（Hosting/Functions ロールバック・DB マイグレーション巻き戻し）の整備 | Codex + Claude | VSCode + Codex | Firebase Hosting rollback / Functions 再デプロイ / Supabase スキーマ巻き戻し手順を docs 化し、T746 Go/No-Go チェックリストへ連結 | 未着手 |
