...
| **T835** | 8. 本番運用・品質管理 | セキュリティ | 公開URLセキュリティヘッダhardening（CSP / X-Content-Type-Options / Permissions-Policy） | Codex + Antigravity | VSCode + Codex + Antigravity + Firebase official docs | T805のMED/LOW検出を受け、Firebase Hostingの本番URLへCSP、X-Content-Type-Options、Referrer-Policy、Permissions-Policy、frame-ancestors / X-Frame-Options、HSTSを設定。GitHub Pagesは任意HTTPヘッダを設定できないためcontrolled demo mirrorとして制約と代替方針をdocsへ記録。scripts/validate_firebase_hosting_headers.py、exports/firebase_hosting_headers_review.*、pytestでR94/SEC-008を解決し、public_paid_launchのセキュリティヘッダ判定へ反映 | 完了 |
