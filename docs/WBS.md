...
| **T680** | 6. 社長プレゼン準備 | UI・動画リソース | 最上部動画のSeedance API動画固定と下部詳細プレイヤーの別動画化（procedural fallback） | AIエージェント | Antigravity + Gemini | 最上部背景動画をSeedance API製MP4に固定し、下部詳細動画とダウンロード用ソースに別ビデオを適用して完全非同期化をビジュアル検証する |
