...
| **T786** | 7. 決定後実行 | インフラ | GitHub Actions ランナー Node 24 デフォルト切替（2026-06-16）への対応 | Codex | VSCode + Codex + GitHub Actions | `.github/workflows/` のサードパーティ action を v4 系へ点検し、FORCE_JAVASCRIPT_ACTIONS_TO_NODE24=true で事前検証して切替日前に対応完了させる | 未着手 |
...
| **T788** | 7. 決定後実行 | インフラ設計 | ステージング環境（Firebase Hosting preview channel / Supabase 検証用プロジェクト）の構築と運用ルール整備 | Codex | VSCode + Codex | 本番反映前の preview channel デプロイ手順と staging/prod の JWT secret・DB 分離ルールを確立し docs 同期 | 未着手 |
