...
| **T835** | 8. 本番運用・品質管理 | セキュリティ | 公開URLセキュリティヘッダhardening（CSP / X-Content-Type-Options / Permissions-Policy） | Codex + Antigravity | VSCode + Codex + Antigravity + Firebase official docs | T805のMED/LOW検出を受け、Firebase Hostingの本番URLにCSP、X-Content-Type-Options、Referrer-Policy、Permissions-Policy、frame-ancestors等を設定し、GitHub Pages側は制約と代替方針をdocsへ記録。R94/SEC-008を解決し、external_pentest_reviewを再実行してpublic_paid_launch判定へ反映 | 未着手 |
