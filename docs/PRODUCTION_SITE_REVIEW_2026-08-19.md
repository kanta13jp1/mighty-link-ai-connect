...
- 初期セットアップ画面の`section`開始タグ欠落と余分な終了タグにより、後続画面が`.global-main-area`の外へ出ていた。DOM階層を復元し、全画面をアプリシェル内へ戻した。
- ライトテーマで暗色パネルと暗色文字が混在していた。コンテンツ面、入力欄、ボタン、固定ナビゲーションのテーマ別トークンを整理した。
- 固定ヘッダー下の過剰余白、管理画面の2表、研修資料の長いリンクが画面幅を超えていた。グリッドの最小幅、折り返し、モバイル余白を修正した。
- GitHub ActionsのPlaywright導入がUbuntuの`apt`ミラー待ちで停止していた。GitHubホストランナー既存の実行ライブラリを利用し、Chromiumだけを導入するよう変更した。

### 本番検証

| 確認項目 | 結果 |
| :-- | :-- |
| Basic Auth未認証 | 401を維持 |
| ローカルで安全に管理した有効な資格情報 | 200、サーバー認証済みマーカーあり、重複ログイン画面なし |
| 8画面 × 1440px/390px × ライト/ダーク | 32状態すべて表示、アプリシェル内、横方向のはみ出しなし |
| 実通信ブラウザ監査 | コンソールエラー0件、JavaScript例外0件 |
| 公開デモガード | PASS |
| ローカル完全プリフライト | ガード29/29、テスト911/911、失敗0、エラー0 |

- アプリ修正: `023f672c`
- CI安定化: `1a4c454c`
- [GitHub Actions デプロイ実行](https://github.com/kanta13jp1/mighty-link-ai-connect/actions/runs/32277276051): 成功
- 画面証跡: [ライト・デスクトップ](../exports/production_review_2026-08-20/production_light_desktop.png)、[ダーク・モバイル](../exports/production_review_2026-08-20/production_dark_mobile.png)
