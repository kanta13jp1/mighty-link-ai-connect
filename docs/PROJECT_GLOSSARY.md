...
| **RAG** | **RAG (Retrieval-Augmented Generation / 検索拡張生成)** | - | LLMの生成能力と外部ナレッジ検索を組み合わせ、ハルシネーションを抑制し最新・高精度な回答を構築するアーキテクチャ。営業メールAIやNotebookLM連携で使用。 |
| **POP3** | **POP3 (Post Office Protocol v3)** | - | メールサーバーから受信用クライアント/自動処理プログラムへ電子メールを取得・ダウンロードする標準プロトコル。営業メールAIでの案件・人材メール全件取り込みで使用。 |
| **LLM** | **LLM (Large Language Model / 大規模言語モデル)** | - | 膨大なテキストデータから学習したAI基盤モデル。本プロダクトでは Gemini シリーズ（1.5 / 2.0 / 3.1 等）を採用し、メール解析・診断評価等に活用。 |
...
| **ハーネス** | Test Harness | プログラムやAIモデルの自動テスト・評価を行うための実行枠組み（テストドライバ、モック、アサーション、証跡出力等）。プリフライト監査やPytestスイート等で活用。 |
| **セッション** | Session | 一連の継続的な処理・対話単位。ユーザーの認証維持状態（Firebase Auth）および1回の開発作業サイクル（AGENTS.mdセッション規約）を指す。 |
