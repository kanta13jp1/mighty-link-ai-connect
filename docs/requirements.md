...
2026-06-17の小林社長・梅澤打ち合わせで、営業全員が見られる共有営業アドレスに毎日約1,000通届く営業メール、案件メール、要員情報メールから案件要件や人材情報を自動抽出し、エンジニア候補とマッチングする機能を最優先で開発する方針に更新した。

- 受信環境をヒアリングしたうえで、Microsoft Graph、Gmail API、IMAP、Webhook/メール転送、ファイル監視、手動アップロードのいずれかで営業メールを取り込む。
- AIが必須スキル、尚可スキル、勤務地、リモート条件、単価、稼働時期、商流などを構造化する。
- Supabaseへ案件メールDBを作り、重複排除、検索、レビュー、マッチング結果を保存する。
- エンジニア/経歴書から案件を探す方向と、案件要件から候補人材を探す方向の両方を対象にする。
- SQLやOracleのような過剰ヒットしやすい語は、単純キーワード検索ではなく文脈と条件を構造化して判定する。
