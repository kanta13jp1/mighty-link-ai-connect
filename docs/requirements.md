...
### 2.2.1 【最優先MVP】営業メールAIマッチング

2026-06-17の小林社長・梅澤打ち合わせで、毎日約1,000通届く営業メール、案件メールから案件要件を自動抽出し、エンジニア候補とマッチングする機能を最優先で開発する方針に更新した。

- Gmail APIまたは手動アップロードで営業メールを取り込む。
- AIが必須スキル、尚可スキル、勤務地、リモート条件、単価、稼働時期、商流などを構造化する。
- Supabaseへ案件メールDBを作り、重複排除、検索、レビュー、マッチング結果を保存する。
- スコアだけでなく、メール本文由来の根拠、不一致理由、人間レビュー結果を表示する。
- 本文全文や認証情報は、GitHub、Sheets、Issue、NotebookLMへ転載しない。

詳細は [SALES_EMAIL_AI_MATCHING_REQUIREMENTS.md](SALES_EMAIL_AI_MATCHING_REQUIREMENTS.md) を正本とする。
