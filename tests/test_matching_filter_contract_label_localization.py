...
def test_matching_filter_contract_label_has_localized_contract():
    expected_markup = (
        '<label for="matching-filter-contract" '
        'data-i18n="matching_filter_contract_label">契約形態</label>'
    )
    for html_file in HTML_FILES:
        html = html_file.read_text(encoding="utf-8")

        assert html.count(expected_markup) == 1
        for label in CONTRACT_LABELS.values():
            assert f'matching_filter_contract_label: "{label}"' in html


def test_matching_filter_contract_label_tracks_language_and_selection(fastapi_server):
    with sync_playwright() as playwright:
        ...
        label = page.locator('label[for="matching-filter-contract"]')
        select = page.locator("#matching-filter-contract")
        select.select_option("派遣")

        for language, expected_label in CONTRACT_LABELS.items():
