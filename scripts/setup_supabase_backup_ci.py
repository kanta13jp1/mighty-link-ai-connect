...
DEFAULT_BUCKET_NAME = "mightylink-supabase-backup"
...
def generate_setup_plan(
    ...
    commands = [
        ...
        f"gcloud iam service-accounts add-iam-policy-binding {sa_email} --project={project_id} --role='roles/iam.workloadIdentityUser' --member='principalSet://iam.googleapis.com/projects/{project_number}/locations/global/workloadIdentityPools/{pool_id}/attribute.repository/{repo}'",
        f"gcloud storage buckets add-iam-policy-binding gs://{bucket_name} --member='serviceAccount:{sa_email}' --role='roles/storage.objectAdmin'",
        f"",
        ...
        f"gh secret set GCP_WORKLOAD_IDENTITY_PROVIDER --repo {repo}",
        f"gh secret set GCP_SERVICE_ACCOUNT_EMAIL      --repo {repo}",
