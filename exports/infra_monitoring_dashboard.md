...
| --- | --- | --- | --- | --- |
| host | disk_usage | warning | 80.66% used (90191.2 MB free / 466325.0 MB total) | Investigate generated artifacts, logs, and old archives if disk usage exceeds 80%; set INFRA_HOST_RESOURCE_CRITICAL=1 to fail local/CI runner saturation. |
| host | memory_usage | warning | 85.00% used (2338.0 MB available / 16068.7 MB total) | Use Google Cloud Monitoring / Cloud Run metrics for production memory saturation; set INFRA_HOST_RESOURCE_CRITICAL=1 to fail local/CI runner saturation. |
...
| host | repo_data_exports_size | ok | data=0.7 MB, exports=71.7 MB | Keep generated artifacts bounded; large binary growth should move to Drive/GCS. |
| availability | uptime_summary | ok | ok=3 warning=0 failed=0 | Investigate failed targets; TLS warnings for mightylink-app.com remain expected until T740_3. |
| availability | uptime_max_latency | ok | 321.87 ms | Correlate high latency with Firebase Hosting and Cloud Run metrics. |
...
| cost | external_api_usage | ok | events=59, billable=0, blocked=59 | Compare billable usage with T757 cost dashboard and provider consoles. |
