...
- 判定: **ok** (10/10 仮説PASS)
- WBS完了率: **96.9%**（完了344 / 実行中0 / 未着手11 / 総数355）
- 期限超過(未完了): なし
...
## 再評価候補ゲート（関連WBS全完了かつ未解決課題なし・PASS再判定推奨）: PUBLIC-06, PUBLIC-14
...
| PUBLIC-08 | WARNING | — | R143 | blocked_by_open_issue |
| PUBLIC-09 | BLOCKED | T791, T807 | R143 | lane |
| PUBLIC-11 | BLOCKED | — | R132 | blocked_by_open_issue |
| PUBLIC-13 | BLOCKED | T845, T849 | R122 | human_or_mixed |
...
| # | 仮説 | 結果 | 根拠 |
| --- | --- | --- | --- |
| H1 | WBSが解析でき、ステータス内訳の合計が総数に一致する | PASS | 完了344+実行中0+未着手11=355 |
| H2 | 完了率が算出され、9割超である | PASS | 完了率=96.9%（344/355） |
...
| H8 | 各非PASSゲートが残タスク/HUMAN_GATE/未解決課題/再評価候補として説明できる | PASS | 説明不能な非PASSゲート=なし / 再評価候補(関連全完了かつ未解決課題なし)=['PUBLIC-06', 'PUBLIC-14'] / 未解決課題でPASS再判定不可={'PUBLIC-08': ['R143'], 'PUBLIC-11': ['R132']} |
| H9 | 残作業が人間依存/レーン/再評価候補に分類できる | PASS | 残作業の分類=['blocked_by_open_issue', 'human_or_mixed', 'lane', 'reevaluate_candidate'] |
| H10 | 集約出力が内部整合している（完了数の再計算一致） | PASS | 完了数再計算=344(=344) |
