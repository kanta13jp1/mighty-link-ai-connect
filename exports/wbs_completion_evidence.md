...
- レポートID: `WBS_COMPLETION_EVIDENCE_T849_1` / 実施日: 2026-09-05
- 判定: **ok** (10/10 仮説PASS)
- WBS完了率: **97.8%**（完了435 / 実行中7 / 未着手3 / 総数445）
- 期限超過(未完了): T823, T831, T834, T845, T849, T850, T911, T944, T957, T977
...
| # | 仮説 | 結果 | 根拠 |
| --- | --- | --- | --- |
| H1 | WBSが解析でき、ステータス内訳の合計が総数に一致する | PASS | 完了435+実行中7+未着手3=445 |
| H2 | 完了率が算出され、9割超である | PASS | 完了率=97.8%（435/445） |
| H3 | 期限超過の未完了タスクを検出できる（可視化対象） | PASS | 期限超過(未完了)=['T823', 'T831', 'T834', 'T845', 'T849', 'T850', 'T911', 'T944', 'T957', 'T977'] |
...
| H10 | 集約出力が内部整合している（完了数の再計算一致） | PASS | 完了数再計算=435(=435) |
