...
- 判定: **ok** (10/10 仮説PASS)
- WBS完了率: **94.6%**（完了318 / 実行中0 / 未着手18 / 総数336）
- 期限超過(未完了): なし
...
| PUBLIC-13 | BLOCKED | T845, T849, T850 | R116, R122 | human_or_mixed |
...
| # | 仮説 | 結果 | 根拠 |
| --- | --- | --- | --- |
| H1 | WBSが解析でき、ステータス内訳の合計が総数に一致する | PASS | 完了318+実行中0+未着手18=336 |
| H2 | 完了率が算出され、9割超である | PASS | 完了率=94.6%（318/336） |
...
| H8 | 各非PASSゲートが残タスク/HUMAN_GATE/未解決課題/再評価候補として説明できる | PASS | 説明不能な非PASSゲート=なし / 再評価候補(関連全完了かつ未解決課題なし)=なし / 未解決課題でPASS再判定不可={'PUBLIC-11': ['R132'], 'PUBLIC-14': ['R116']} |
| H9 | 残作業が人間依存/レーン/再評価候補に分類できる | PASS | 残作業の分類=['blocked_by_open_issue', 'human_or_mixed', 'lane'] |
| H10 | 集約出力が内部整合している（完了数の再計算一致） | PASS | 完了数再計算=318(=318) |
